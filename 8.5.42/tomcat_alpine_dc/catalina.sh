...
#set runtime options 
PINPOINT_OPTS=" -javaagent:/usr/local/tomcat/pinpoint/pinpoint-bootstrap-1.8.0.jar -Dpinpoint.applicationName=${_SYS}-${_SAPP}-${_SGRP}${_ITG} -Dpinpoint.agentId=${_SGRP}${_ITG}-${HOST} "
DUMP_OPTS=" -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/usr/local/tomcat/logs/DUMP_"$HOST"."$(date +%Y%m%d_%H%M%S)
JAVA_OPTS=" -server -Duser.timezone=GMT+08  "
JAVA_OPTS=$JAVA_OPTS" "$MEM_ARGS" "$JVM_OPTS" "$HOST_OPTS$PINPOINT_OPTS$DUMP_OPTS
...
if [ -z "$CATALINA_OUT" ] ; then
  CATALINA_OUT="$CATALINA_BASE"/logs/catalina_${HOST}.out
fi
